module Refinery
  module Admin
    class UsersController < Refinery::AdminController
      ...
      def new
        @user = Refinery.user_class.new
        @selected_plugin_names = []
      end

      def create
        @user = Refinery.user_class.new(params[:user].except(:roles))
        ...
        if @user.save
          ...
          unless refinery_current_user.has_refinery_role?(:superuser) && Refinery::Authentication.superuser_can_assign_roles
      ...
      def update
        ...
        if refinery_current_user.id == @user.id && # If editing self
    ...
    protected
      ...
      def find_user_with_slug
        @user ||= begin
          ...
          @user = Refinery.user_class.all.detect{|u| u.to_param == params[:id]}
        end
      end
      ...
      def redirect_unless_user_editable!
        unless refinery_current_user.can_edit_refinery_user? find_user
